...
cordata
nummerischecolnames <- paste(toString(colnames(cordata[,-14])))
nummerischecolnames
...
for (i in 1:maxdegree){
  ...
  glm.polynomial.fit[[i]] <- glm(   
    formula =  G_average ~ poly(age, Medu, Fedu, traveltime, studytime, failures, famrel, freetime, goout, Dalc, Walc, health, absences, degree=i, raw=TRUE),
    data    = traindata
  )
...
####################################################################################seed 41
set.seed(41) # Zufallsparameter auf 41
trainingsrows <- sample(nrow(port), nrow(port)*0.8)    # 80% der Gesamtdaten als Trainingsdaten
traindata <- port[trainingsrows,]
testdata <- port[-trainingsrows,]
...
#####################################################################################################seed42
set.seed(100) # Zufallsparameter auf 42
trainingsrows <- sample(nrow(port), nrow(port)*0.8)    # 80% der Gesamtdaten als Trainingsdaten
traindata <- port[trainingsrows,]
testdata <- port[-trainingsrows,]
